...
void SudokuSolver::search(int k) {
  ...
  printf("before for loop in search\n");
  printf(column->down == column ? "wtf\n" : "why no work\n");
  ...
  printf("after for loop in search\n");
...
SudokuSolver::SudokuSolver(char *filename, int N)
    : filename(filename)
    , N(N)
    , nResult(0)
    , maxK(0)
